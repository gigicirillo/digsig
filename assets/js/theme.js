(function(){
  ...
  function init(){let saved='dark';try{saved=localStorage.getItem(KEY)||'dark'}catch(e){}apply(saved);enhance();addLogout();verify();loadScript('lyncaDuplicateScript','assets/js/duplicate.js?v=20260914-2135');loadScript('lyncaFullscreenDesignScript','assets/js/fullscreen-design-control.js?v=20260914-2150');document.addEventListener('submit',handleScreen,true);document.addEventListener('click',e=>{const b=e.target.closest('[data-theme-choice]');if(b)apply(b.dataset.themeChoice)});new MutationObserver(enhance).observe(document.body,{childList:true,subtree:true});window.digsigSetDashboardTheme=apply}
