(function(){
  ...
  function loadDuplicate(){if(document.getElementById('lyncaDuplicateScript'))return;const s=document.createElement('script');s.id='lyncaDuplicateScript';s.src='assets/js/duplicate.js?v=20260914-2135';document.body.appendChild(s)}
  function init(){let saved='dark';try{saved=localStorage.getItem(KEY)||'dark'}catch(e){}apply(saved);enhance();addLogout();verify();loadDuplicate();document.addEventListener('submit',handleScreen,true);document.addEventListener('click',e=>{const b=e.target.closest('[data-theme-choice]');if(b)apply(b.dataset.themeChoice)});new MutationObserver(enhance).observe(document.body,{childList:true,subtree:true});window.digsigSetDashboardTheme=apply}
