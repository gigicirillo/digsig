(function(){
  ...
  function hasState(){return typeof state!=='undefined'&&state&&state.config}
  ...
  function closeMediaPreview(){var d=document.getElementById('mediaPreviewModal');if(!d)return;var f=document.getElementById('mediaPreviewFrame');f.src='about:blank';f.removeAttribute('srcdoc');d.close()}
  function openHtmlPreview(code,title){ensurePreviewDialog();var f=document.getElementById('mediaPreviewFrame');f.removeAttribute('src');document.getElementById('mediaPreviewTitle').textContent=title||'HTML';document.getElementById('mediaPreviewMeta').textContent='HTML · preview';f.srcdoc=code||'<body style="background:#111;color:#fff;font-family:Arial;display:grid;place-items:center;height:100vh;margin:0">HTML vuoto</body>';document.getElementById('mediaPreviewModal').showModal()}
  function mediaPreview(id){
    if(!hasState())return;var m=state.config.media.find(function(x){return x.id===id});if(!m)return;ensurePreviewDialog();
    var f=document.getElementById('mediaPreviewFrame'),title=document.getElementById('mediaPreviewTitle'),meta=document.getElementById('mediaPreviewMeta');f.removeAttribute('srcdoc');f.src='about:blank';title.textContent=m.name||'Media';meta.textContent=(m.type||'media')+' · '+(m.type==='video'&&!m.customDuration?'durata naturale':((Number(m.duration)||15)+'s'));
    if(m.type==='rawhtml'){f.removeAttribute('src');f.srcdoc=m.htmlCode||''}
    else if(m.type==='html'){f.removeAttribute('src');f.srcdoc='<!doctype html><html><body style="margin:0;background:'+(m.background||'#111')+';color:#fff;font-family:Inter,Arial,sans-serif;display:grid;place-items:center;height:100vh;text-align:center"><div><h1 style="font-size:7vw;margin:0">'+(m.headline||'')+'</h1><p style="font-size:2vw;opacity:.8">'+(m.subheadline||'')+'</p></div></body></html>'}
    else if(m.type==='image'){f.removeAttribute('src');f.srcdoc='<!doctype html><html><body style="margin:0;background:#000;height:100vh"><img src="'+escAttr(m.url)+'" style="width:100%;height:100%;object-fit:contain"></body></html>'}
    else if(m.type==='video'){var yt=String(m.url||'').match(/(?:youtube\.com\/watch\?[^#]*v=|youtu\.be\/|youtube\.com\/embed\/|youtube\.com\/shorts\/|youtube\.com\/live\/)([A-Za-z0-9_-]{6,})/i);if(yt)f.src='https://www.youtube-nocookie.com/embed/'+encodeURIComponent(yt[1])+'?autoplay=1&mute=1&controls=0&rel=0&playsinline=1';else{f.removeAttribute('src');f.srcdoc='<!doctype html><html><body style="margin:0;background:#000;height:100vh"><video src="'+escAttr(m.url)+'" autoplay muted controls style="width:100%;height:100%;object-fit:contain"></video></body></html>'}}
    else if(m.type==='web')f.src=m.url;else{f.removeAttribute('src');f.srcdoc=''}
    document.getElementById('mediaPreviewModal').showModal()
  }
  window.mediaPreview=mediaPreview;
  function enhanceCards(){
    var box=document.getElementById('mediaCards');if(!box||!hasState())return;var cards=box.querySelectorAll('.card');
  ...
  function init(){ensureHtmlOption();ensureEditor();ensurePreviewDialog();patchMediaFunctions();var type=document.getElementById('mediaType');if(type)type.addEventListener('change',syncFields);var cards=document.getElementById('mediaCards');if(cards)new MutationObserver(enhanceCards).observe(cards,{childList:true,subtree:true});enhanceCards();setInterval(function(){patchMediaFunctions();enhanceCards()},800)}
